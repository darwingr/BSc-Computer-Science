...
int main(int argc, char **argv)
{
    ...
    MPI_Barrier(MPI_COMM_WORLD); // for precise timing
    ...
    if (id == 0) {
        ...
        // we do the calculation for every point of our complex plane,
        // thus on our 2D image with appropriate steps
        ...
        for (int i=1; i<nproc; ++i) {
            MPI_Recv(answer, 2, MPI_INT, MPI_ANY_SOURCE, 1, MPI_COMM_WORLD,
                    &status);
            MPI_Recv(&img_array[answer[1]][0], imgY, MPI_INT, answer[0], 2,
        ...
        // we get another time at this point, so we can calculate the elapsed
        // time for the calculation
        e_time = MPI_Wtime();

        cout << "Time elapsed during calculation: "
            << e_time-s_time << " secs."
            << endl;;
        ...
        // We have to colour our dataset. Actually, the members of the
        // Mandelbrot set are used to be the same colour (black?) and have from
        // point of visualisations view no interest.
        //
        // The outer points are represented by assigning colours to their
        // iteration steps and this generates vivid forms and colors.
        for (int i=0; i<imgX; i++) {

            for (int j=0; j<imgY; j++) {
                // We go from black to red in this range
                if ( img_array[i][j] < 256) {
                    myfile << img_array[i][j] << " 0 0";
                    // (int)(84*pow(img_array[i][j],0.2)) << " 0 0";
                    // //myfile << img_array[i][j] << " 0 0";
                }
                // we go from red to yellow in this range
                else if ( img_array[i][j] < 512) {
                    myfile << "255 " << img_array[i][j]-256 << " 0";
                }
                // we go from yellow to white in this range
                else if ( img_array[i][j] < 768) {
                    myfile << "255 255 " << img_array[i][j]-512;
                }

                /*
                // we could refine our palette for more resolution,
                //  more iteration-step images
        ...
        // Give another elapsed time info (IO included)
        e_time = MPI_Wtime();
    ...
    // Slave
        ...
        while (1) {
            ...
            // at every new step in X direction, we start at the first Y value
            cy = yMin;
...
//
// it will get two parameters (x and y coordinates) and will give an iteration
// count in return
int converges (double cx, double cy) {
    int n=0;
    double zx=0;
    double new_zx=0;
    double zy=0;
    // we iterate until max. iteration count iter_n, or until z^2 (complex!)
    // runs over 4 - this means, our series will run to infinity,
...
void usage(std::string program) {
    cout << "Usage: " << endl;
    cout << program << " out_file.ppm" << endl;
